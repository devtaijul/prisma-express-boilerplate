...
// Upload single file
router.post("/upload", fileUpload_1.uploadSingle, async (req, res, next) => {
    try {
        if (!req.file) {
        ...
        // Get file URL
        const fileUrl = (0, fileUpload_1.getFileUrl)(filename, year, month);
        // Get image dimensions if it's an image
        let dimensions = { width: 0, height: 0 };
        if (req.file.mimetype.startsWith("image/")) {
            dimensions = await (0, fileUpload_1.getImageDimensions)(filePath);
        }
...
router.post("/upload-multiple", fileUpload_1.uploadMultiple, async (req, res, next) => {
    try {
...
router.get("/", async (req, res, next) => {
    try {
        ...
        // Get total count
        const total = await prisma_1.prisma.media.count({ where });
...
router.delete("/:id", async (req, res, next) => {
    try {
        ...
        if (urlParts.length > 1) {
            const filePath = path_1.default.join(process.env.UPLOAD_PATH || "./uploads", urlParts[1]);
